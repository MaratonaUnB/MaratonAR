...
const cleanupAR = () => {
  ...
  document.querySelectorAll('video').forEach((v: any) => {
    const s = v.srcObject; if (s) s.getTracks().forEach((t: any) => t.stop()); v.srcObject = null; v.remove()
  })
...
const loadScript = (src: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    const s = document.createElement('script'); s.src = src; s.onload = () => resolve(); s.onerror = () => reject(new Error('Erro script')); document.head.appendChild(s)
  })
}
      ...
      <div v-if="currentTarget" class="event-overlay">
        <div class="terminal-card">
          ...
          <div class="action-footer">
            <button class="download-btn" @click="downloadFile('IIMaratonaCerrado.jpg')">
              ...
              <span>DOWNLOAD IMAGEM PARA INSTAGRAM</span>
            </button>
          </div>
        </div>
      </div>
